...
export default function Rider(){
 ...
 return <main className="riderFaiPage"><div className="riderFaiShell">
  ...
  {view==='orders'&&<section className="riderView"><div className="riderViewTitle"><div><h2>New / Active Delivery</h2><p>Only orders assigned by Mahi Eats Super Admin</p></div><span>{orders.length}</span></div>{loading&&!orders.length?<div className="riderEmpty">Loading deliveries...</div>:orders.length===0?<div className="riderEmpty"><Bike/><b>No new delivery</b><p>New assigned order yahan automatically show hoga.</p></div>:orders.map(o=><article className="riderFaiOrder" key={o.id}>
   <div className="riderOrderTop"><div><small>ORDER</small><b>#{o.id}</b></div><span className={`riderStatus ${o.rider_status}`}>{String(o.rider_status||'assigned').replaceAll('_',' ')}</span></div>
   ...
   <div className="riderRouteCard"><div><Store/><span><b>Pickup</b><small>{o.shop?.address||o.shop?.name}</small></span><a target="_blank" rel="noreferrer" href={mapUrl(o.shop?.latitude,o.shop?.longitude,o.shop?.address||o.shop?.name)}>Map</a></div><div><MapPin/><span><b>Deliver to {o.customer_name}</b><small>{o.delivery_address||'Customer location'}</small></span>{o.customer_latitude!=null&&<a target="_blank" rel="noreferrer" href={mapUrl(o.customer_latitude,o.customer_longitude,o.delivery_address)}>Map</a>}</div></div>
   <div className="riderCustomerLine"><a href={`tel:${o.customer_phone}`}><Phone/> {o.customer_phone}</a>{o.delivery_distance_km!=null&&<span>{Number(o.delivery_distance_km).toFixed(2)} km</span>}</div>
   ...
   <div className="riderMainActions">{o.customer_latitude!=null&&<a className="riderNavigate" target="_blank" rel="noreferrer" href={mapUrl(o.customer_latitude,o.customer_longitude,o.delivery_address)}><Navigation/>Navigate</a>}
