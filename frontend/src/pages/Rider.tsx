...
export default function Rider(){
 ...
 return <main className="riderFaiPage"><div className="riderFaiShell">
  ...
  {view==='orders'&&<section className="riderView"><div className="riderViewTitle"><div><h2>New / Active Delivery</h2><p>Only orders assigned by Mahi Eats Super Admin</p></div><span>{orders.length}</span></div>{loading&&!orders.length?<div className="riderEmpty">Loading deliveries...</div>:orders.length===0?<div className="riderEmpty"><Bike/><b>No new delivery</b><p>New assigned order yahan automatically show hoga.</p></div>:orders.map(o=><article className="riderFaiOrder" key={o.id}>
   ...
   <div className="riderCustomerContact"><div><b>Customer contact</b><strong>{o.customer_name}</strong><span>{o.customer_phone}</span></div><div><a href={`tel:${o.customer_phone}`}><Phone/>Call</a><a target="_blank" rel="noreferrer" href={`https://wa.me/${wa(o.customer_phone)}`}><MessageCircle/>WhatsApp</a><a target="_blank" rel="noreferrer" href={mapUrl(o.customer_latitude,o.customer_longitude,o.delivery_address||'Customer location')}><Navigation/>Navigate</a></div>{o.delivery_distance_km!=null&&<small>{Number(o.delivery_distance_km).toFixed(2)} km road distance</small>}{o.customer_latitude==null&&<small className="gpsMissing">Exact GPS was not saved on this older order; map will search the saved address.</small>}</div>
