...
export default function CustomerOrders(){
 ...
 return <main className="page customerOrdersPage customerAppWithNav"><div><Link className="back" to="/"><ArrowLeft/>Home</Link><div className="eyebrow">YOUR MAHI EATS</div><h1>Orders</h1></div>{msg&&<div className="error">{msg}</div>}<div className="customerOrderList">{orders.map(o=><article className="customerOrderCard" key={o.id}><div className="customerOrderTop"><div className="miniLogo"><Store/></div><div><h3>{o.shop?.name||'Shop'}</h3><span>Order #{o.id}</span></div><span className={`statusPill ${o.status}`}>{o.status}</span></div><div className="customerOrderMeta"><span><Clock3/> {new Date(o.created_at).toLocaleString('en-AE',{timeZone:'Asia/Dubai'})}</span><span><MapPin/> {o.delivery_address||'Delivery address'}</span></div><div className="row"><b>AED {Number(o.total||0).toFixed(2)}</b><Link className="small buttonLink" to={`/track/${o.id}?phone=${encodeURIComponent(o.customer_phone)}`}>View details</Link></div></article>)}{!orders.length&&!msg&&<div className="marketEmpty"><ReceiptText/><h3>No orders yet</h3><p>Your Mahi Eats orders will appear here.</p><Link className="buttonLink" to="/">Browse shops</Link></div>}</div><CustomerBottomNav active="orders"/></main>;
}
